...
def participant_page():
    ...
    with st.sidebar:
        ...
        selected = option_menu(
            menu_title=None,
            options=["My Activities", "Team Hub", "Explore & Register", "Messages","View Announcemnets & Results", "My Profile", "About Us"],
...
# --- 3. EXPLORE & REGISTER (Colored Square Cards) ---
    ...
    elif selected == "View Announcemnets & Results":
    # --- HEADER WITH VIEW RESULT BUTTON ---
         col_title, col_btn = st.columns([3, 1])
    
         with col_title:
             st.title("🏆 Hackathon Results & Shortlists")
    
         with col_btn:
        # Top right button
             show_result_modal = st.button("📊 View Leaderboard", use_container_width=True)

    # --- 1. MODAL/LOGIC FOR LEADERBOARD SEARCH ---
         if show_result_modal or st.session_state.get('searching_result', False):
             st.session_state['searching_result'] = True
        
             with st.container():
                 st.markdown("### 🔍 Search Leaderboard")
                 col1, col2 = st.columns(2)
                 search_h_name = col1.text_input("Enter Hackathon Name")
                 search_t_name = col2.text_input("Enter Your Team Name")
            
                 if st.button("Check Result"):
                # VALIDATION: Check if team belongs to this hackathon
                     check_team = execute_query("""
                         SELECT t.team_id, t.hackathon_id 
                         FROM team t 
                         JOIN hackathon_table h ON t.hackathon_id = h.hackathon_id
                         WHERE h.hackathon_name = %s AND t.team_name = %s
                     """, (search_h_name, search_t_name), fetch=True)
                
                     if check_team:
                         h_id = check_team[0]['hackathon_id']
                    
                    # FETCH LEADERBOARD (Same as Judge logic)
                         leaderboard_query = """
                             SELECT 
                                 t.team_name, 
                         ...
                         if results:
                             st.success(f"✅ Results found for {search_h_name}!")
                             import pandas as pd
                        ...
                        # Winner Announcement
                             st.balloons()
                             st.info(f"🥇 Winner: **{results[0]['team_name']}**")
                         else:
                             st.warning("Scores for this hackathon haven't been finalized yet.")
                     else:
                         st.error("❌ Invalid Combination: This team is not registered for the mentioned hackathon.")
            
                 if st.button("Close Search"):
                     st.session_state['searching_result'] = False
                     st.rerun()

         st.write("---")
         st.write("Below are the details of the shortlisted teams.")

    # --- 2. EXISTING SHORTLIST DATA FETCHING ---
         ...
         if shortlisted_data:
             categories = {"upcoming": {}, "ongoing": {}, "evaluated": {}}
             ...
             for status_key in display_order:
                 if status_key in categories and categories[status_key]:
                     found_match = True
                     icon = "📅" if status_key == "upcoming" else "🚀" if status_key == "ongoing" else "🏁"
                     st.markdown(f"## {icon} {status_key.capitalize()} Hackathons")
                
                     for h_name, teams in categories[status_key].items():
                         is_expanded = (status_key == "evaluated" or status_key == "ongoing")
                         with st.expander(f"Detailed Shortlist: {h_name} ({len(teams)} Teams)", expanded=is_expanded):
                             import pandas as pd
                             df = pd.DataFrame(teams)[['team_id', 'team_name']]
                             df.columns = ['Team ID', 'Shortlisted Team Name']
                             st.dataframe(df, use_container_width=True, hide_index=True)
                     st.markdown("---")

             if not found_match:
                 st.warning("No matched hackathon statuses found.")
             else:
                 st.info("No shortlists announced yet. ⏳")

    # --- 3. SIDEBAR NOTIFICATION ---
             with st.sidebar:
                 st.markdown("---")
                 st.info("💡 **Note:** Result checking requires your Team Name and Hackathon Name to match our records.")
    # --- 5. MY PROFILE ---
