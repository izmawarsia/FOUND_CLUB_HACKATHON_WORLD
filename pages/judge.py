            ...
            for team in teams_data:
                ...
                with st.expander(f"👥 Team: {team['team_name']}"):
                    ...
                    if has_scores:
                        ...
                        if st.button(f"🔄 Redo/Update Scores", key=f"redo_{team['team_id']}"):
                             st.session_state[f"edit_mode_{team['team_id']}"] = True

    # Agar update button dabaya ho toh form dikhao
                        if st.session_state.get(f"edit_mode_{team['team_id']}", False):
                             with st.form(key=f"update_form_{team['team_id']}"):
                                 st.markdown("##### 📝 Update Scores")
                                 c1, c2, c3 = st.columns(3)
            # Purani values default set kar di hain
                                 new_ts = c1.number_input("Tech", 0, 10, value=team['technical_score'], key=f"uts_{team['team_id']}")
                                 new_ins = c2.number_input("Inno", 0, 10, value=team['innovative_score'], key=f"uins_{team['team_id']}")
                                 new_es = c3.number_input("Exec", 0, 10, value=team['evaluative_score'], key=f"ues_{team['team_id']}")
            
                                 if st.form_submit_button("Save Changes"):
                                     try:
                                         save_score_query = """
                                             INSERT INTO scores (judge_id, hackathon_id, team_id, project_title, technical_score, innovative_score, evaluative_score)
                                             VALUES (%s, %s, %s, %s, %s, %s, %s)
                                             ON DUPLICATE KEY UPDATE 
                                             technical_score=VALUES(technical_score), 
                                             innovative_score=VALUES(innovative_score), 
                                             evaluative_score=VALUES(evaluative_score)
                                         """
                                         execute_query(save_score_query, (
                                             user['user_id'], h_id, team['team_id'], team['project_title'],
                                             new_ts, new_ins, new_es
                                         ))
                                         st.success("Scores Updated Successfully! ✅")
                                         st.session_state[f"edit_mode_{team['team_id']}"] = False
                                         st.rerun()
                    ...
                    # SCENARIO 2: Submitted (Ready for marking)
                        ...
                        with st.form(key=f"mark_form_{team['team_id']}"):
                            ...
                            if st.form_submit_button("Submit Evaluation"):
                                 try:
                                    # Perfecting the mapping based on your DB requirements
                                    save_score_query = """
                                        INSERT INTO scores (
                                            judge_id, 
                                            hackathon_id, 
                                    ...
                                    # Order strictly following the query above
                                    execute_query(save_score_query, (
                                        user['user_id'],       # Current Judge (e.g., U-038)
                                        h_id,                  # Active Hackathon (e.g., H-001)
                                        team['team_id'],       # Team ID from FETCHED data
                                        team['project_title'], # Exact Title from FETCHED data
                                        ts,                    # Tech Score input
                                        es,                    # Exec Score input
                                        ins                    # Inno Score input
                                    ))
                                    
                                    st.success(f"Evaluation Done for team: {team['team_name']}! ✅")
                                    st.balloons()
                                    ...
                                    # Direct error display taake terminal na dekhna parhe
                                    st.error(f"SQL Submission Error: {e}")
